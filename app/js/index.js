(function(){
	...
	// 网点编码
	$body.on('touchstart', '#point-num-show', function(e){
		e.preventDefault();
		status_point_num = 1;
	...
	$body.on('click', '.search-result li', function(){
	...
	// 区域选择
	$body.on('touchstart', '.area-select', function(e){
		var id = $(this).attr('id'),
				html = "",
				cur_title, // 省市区
				area_title, // 省市区
	...
	$body.on(tap, '.area li', function(e){
	...
	var codeHtml = '<li class="code-rm">'
									...
									+ '<div></div>'
								+ '</li>';
	$body.on(tap, '.code-add > div', function(){
		$(this).parent().before(codeHtml);
	});
	$body.on(tap, '.code-rm > div', function(){
		$(this).parent().remove();
	});
	...
	$body.on('focus', '.main input', function(){
		$cur_obj = $(this);
	});
})();